...
- 月〜金から選んだ曜日に通常利用を月単位で更新し、未選択曜日の通常利用だけを解除
- 日付ごとの天気の登録
- 体調、睡眠、疲労、気分、痛み、バイタル、運動、写真付き筋トレ機器、自転車・歩行・階段・温熱・上肢プーリーの時間、補助具、担当者、成果などの記録
- 入力中の内容を端末内へ自動保存（30秒ごとにも保存）
- 記録の詳細表示と、期間別のバイタル・体調・痛み・満足度の推移グラフ
- 利用回数・休み回数・振替回数、よく記録された成果の集計
- 記録の印刷
- JSON 形式でのバックアップ・復元（復元前には現在のデータを自動バックアップ）
- 記録の CSV 出力
- PWA のインストール案内と更新通知
- Supabase Auth によるログインと、PC・スマホ間の自動同期
- オフライン中は端末へ保存し、オンライン復帰後に自動同期

## データの保存場所と端末間同期

データはまず、このブラウザ・端末内の IndexedDB に保存されます。「設定・データ管理」でSupabaseへログインすると、予定、記録、下書き、担当者、記録の選択肢、利用場所がSupabaseにも保存されます。同じメールアドレスへ届く6桁の使い捨てコードでログインしたPC・スマホでは、同じデータを確認・更新できます。

通信できない間もIndexedDBへ記録でき、オンラインへ戻ると自動同期します。両方の端末で同じ項目を変更した場合は、更新日時が新しい内容を採用します。天気APIの取得キャッシュは端末内だけに保存します。

Supabase同期中でも、定期的に「設定・データ管理」からJSONバックアップを保存してください。
...
| `drafts` | 記録入力中の下書き |
| `syncTombstones` | 別端末へ削除を伝える同期情報 |
| Supabase `compass_data` | ログインユーザーごとの同期データ（RLSで本人の行だけ許可） |
...
1. 片方の端末で「設定・データ管理」を開きます。
2. メールアドレスを入力し、「6桁コードをメールで受け取る」を押します。
3. メールに届いた6桁のコードを入力し、「コードを確認して同期」を押します。
4. もう片方の端末でも、同じメールアドレスに届く新しい6桁コードでログインします。
5. ヘッダーに「同期済み」と表示されれば完了です。
...
別のSupabaseプロジェクトを使う場合は、`.env.example`を`.env.local`へコピーし、Project URLとpublishable keyを変更します。`service_role`キーやsecret keyはブラウザへ設定しないでください。
...
> **はじめに**: インストール前に、公開 URL をブラウザで一度開いてください。Supabaseへログインしていないデータは端末内だけに保存されます。ログイン中でも、機種変更や誤操作に備えて定期的に「データ管理」画面からJSONバックアップを保存してください。
